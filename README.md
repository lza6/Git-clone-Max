...
| 失败恢复 | 单个仓库失败不中断整体；网络类失败自动 treeless 部分克隆降级；平台限制错误明确提示不无效重试 |
| 单任务控制 | 进度表支持搜索过滤/表头排序/单仓库暂停，其余任务继续 |
| 子模块 | 设置开关拉取 `--recurse-submodules`，工作区完整 |
| 代理 | 手动填写或一键自动检测（环境变量 / Windows 注册表） |
| 仓库管理 | 标签 / 收藏置顶 / 黑名单（一键更新排除）/ 过期高亮 / 搜索过滤 |
| 统计报表 | 仓库详情统计、统计中心（全局聚合 + 平台分布）、CSV / Markdown 报表导出 |
| 多主题 | Deep / Light / Nord 三套主题即时切换，重启恢复 |
| 剪贴板监听 | 检测到仓库地址自动提示并填入（设置可关） |
| 可靠性 | token 加密落盘、单实例锁、全局异常兜底 `error.log`、settings 备份恢复、数据库自动迁移 |
